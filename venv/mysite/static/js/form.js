$(function() {
    ...
    var userDate = new Date( selected_year, selected_month - 1, selected_date - 3 );
    var wNames = ['(日a)', '(月a)', '(火a)', '(水a)', '(木a)', '(金a)', '(土a)'];
    var w = wNames[userDate.getDay()];
    document.getElementById('week').innerHTML = '<input name="week" id="week2" type="" maxlength="8" class="txt_code"  style="width:20px;" form="kintaitouroku" />';
    document.getElementById('week2').value = w;
    
    // 曜日(変更)：選択された年・月・日に合わせて、適した曜日の値をセットする
    $('#caldate').change(function() {
        
       selected_year = $('#caldate').val();
       selected_month = $('#month').val();
       selected_date = $('#date').val();

       var userDate = new Date( selected_year, selected_month - 1, selected_date );
       var wNames = ['(日a)', '(月a)', '(火a)', '(水a)', '(木a)', '(金a)', '(土a)'];
       var w = wNames[userDate.getDay()];
       document.getElementById('week2').innerHTML = '<input name="week" id="week3" type="" maxlength="8" class="txt_code"  style="width:20px;" form="kintaitouroku"/>';
       document.getElementById('week3').value = w;
